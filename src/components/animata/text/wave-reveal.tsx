...
const createAnimatedNodes = (args: ReducedValue, word: string, index: number): ReducedValue => {
  ...
  "inline-block opacity-0 transition-all ease-in-out fill-mode-forwards",
  {
    ...
    "animate-content-blur animate-reveal-down": !isUp && blur,
    "animate-content-blur animate-reveal-up": isUp && blur,
  },
  args.className,
);
...
export default function WaveReveal({
  ...
  return (
    <div
      className={cn(
        "relative flex flex-wrap justify-center whitespace-pre px-2 text-4xl md:px-6 md:text-9xl text-white",
        className,
      )}
    >
      {nodes}
